...
$url = urldecode($_REQUEST['videoUrl']);
$delogo = in_array($_GET['delogo'], array(0, 1))? $_GET['delogo'] : 0;
$merge = in_array($_GET['merge'], array(0, 1))? $_GET['merge'] : 0;
$coord = $_GET['coord'];
if (!filter_var($url, FILTER_VALIDATE_URL)) {
	exit('输入的地址是个无效的url地址');
}
$common = new Common();
...
if ($className == 'youtube') {
	...
	header("Location:/youtube.php?videoId=".$videoId);
	exit;
